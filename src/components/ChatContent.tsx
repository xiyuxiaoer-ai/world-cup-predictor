      ...
      <div className={`${showSidebar ? 'flex' : 'hidden'} md:flex flex-col w-full md:w-64 shrink-0 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800`}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {[...members.filter(m => m.user_id !== currentUser.id)].sort((a, b) => {
            ...
            const bOnline = onlineIds.has(b.user_id) ? 2 : 0
            const aDM = conversations.some(c => c.type === 'direct' && (c.user1_id === a.user_id || c.user2_id === a.user_id)) ? 1 : 0
            const bDM = conversations.some(c => c.type === 'direct' && (c.user1_id === b.user_id || c.user2_id === b.user_id)) ? 1 : 0
            if (bOnline + bDM !== aOnline + aDM) return (bOnline + bDM) - (aOnline + aDM)
            return (a.profiles?.display_name || a.profiles?.username || '').localeCompare(b.profiles?.display_name || b.profiles?.username || '')
          }).map(m => {
            const isOnline = onlineIds.has(m.user_id)
            const dm = conversations.find(c => c.type === 'direct' && (c.user1_id === m.user_id || c.user2_id === m.user_id))
            const dmUnread = dm ? unreadConvIds.has(dm.id) : false
            const isActiveDM = dm?.id === selectedConvId
            return (
              <button
                key={m.user_id}
                onClick={() => {
                  if (dm) {
                    setSelectedConvId(dm.id)
                    setUnreadConvIds(prev => { const n = new Set(prev); n.delete(dm.id); return n })
                    setShowSidebar(false)
                  } else {
                    startDM(m.user_id)
                  }
                }}
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 ${isActiveDM ? 'bg-amber-50 dark:bg-amber-900/20 border-r-2 border-amber-400' : ''}`}
              >
                <div className="relative shrink-0">
                  ...
                  {dmUnread && <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-red-500 rounded-full border-2 border-white dark:border-gray-800" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                    {m.profiles?.display_name || m.profiles?.username}
                  </div>
                  <div className={`text-xs ${isOnline ? 'text-green-500' : 'text-gray-400 dark:text-gray-500'}`}>
                    {isOnline ? '在线' : '离线'}
                  </div>
                </div>
              </button>
