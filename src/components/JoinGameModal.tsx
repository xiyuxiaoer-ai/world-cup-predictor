  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl border border-gray-100 animate-float-up">
        <h2 className="text-lg font-bold text-gray-900 mb-1">加入 Game</h2>
        <p className="text-gray-500 text-sm mb-4">输入管理员分享的 Game 码（8位）</p>
        <form onSubmit={handleJoin} className="space-y-4">
          {error && <p className="text-red-500 text-sm bg-red-50 rounded-lg p-3">{error}</p>}
          <input type="text" value={code} onChange={e => setCode(e.target.value)} required autoFocus maxLength={36}
            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-gray-900 placeholder-gray-400 focus:outline-none focus:border-amber-500 focus:bg-white font-mono tracking-widest text-center text-lg transition-colors"
