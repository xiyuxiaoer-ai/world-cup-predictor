...
const inputClass = "w-11 text-center bg-gray-100 border border-gray-200 rounded-lg py-1.5 text-gray-900 font-bold focus:outline-none focus:border-amber-500 focus:bg-white transition-colors"
const selectClass = "w-full bg-gray-50 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-gray-900 focus:outline-none focus:border-amber-500 transition-colors"
...
export default function PredictionCard({
  ...
  if (done && prediction) {
    return (
      <div className="bg-white border-2 border-amber-200 rounded-2xl p-4 space-y-2 shadow-sm">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full font-medium">✓ 已提交</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 flex-1 justify-end">
            <span className="text-sm font-semibold text-gray-900">{homeName}</span>
            {homeFlagUrl && <img src={homeFlagUrl} alt="" className="w-6 h-4 object-cover rounded-sm shrink-0" />}
          </div>
          <div className="shrink-0 px-3 text-gray-900 font-bold text-base">{prediction.pred_home_score} – {prediction.pred_away_score}</div>
          <div className="flex items-center gap-1.5 flex-1 justify-start">
            {awayFlagUrl && <img src={awayFlagUrl} alt="" className="w-6 h-4 object-cover rounded-sm shrink-0" />}
            <span className="text-sm font-semibold text-gray-900">{awayName}</span>
          </div>
        </div>
        <p className="text-xs text-gray-400 text-center">竞猜已锁定，不可修改</p>
      </div>
    )
  }

  return (
    <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-4 space-y-3 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex justify-between items-center flex-wrap gap-1">
        ...
        <div className="flex items-center gap-1.5">
          {isDouble && <span className="text-xs text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded font-semibold">🔥 双倍积分</span>}
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1.5 flex-1 justify-end">
          <span className="text-sm font-semibold text-gray-900 text-right">{homeName}</span>
          {homeFlagUrl && <img src={homeFlagUrl} alt="" className="w-6 h-4 object-cover rounded-sm shrink-0" />}
        </div>
        ...
        <div className="flex items-center gap-1.5 flex-1 justify-start">
          {awayFlagUrl && <img src={awayFlagUrl} alt="" className="w-6 h-4 object-cover rounded-sm shrink-0" />}
          <span className="text-sm font-semibold text-gray-900 text-left">{awayName}</span>
        </div>
      </div>

      {showExtraFields && (
        <div className="space-y-2 border-t border-gray-100 pt-3">
          <p className="text-xs text-gray-500 font-medium">90分钟平局时需猜加时赛：</p>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">加时赛胜者</label>
          ...
          {showPenalty && (
            <div>
              <label className="text-xs text-gray-400 mb-1 block">点球胜者</label>
